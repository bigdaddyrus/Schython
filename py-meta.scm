;Person A: Ruhao Xia , Person B: Lorraina
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
;; File: PY-META.SCM
  ...
  (if (ask line-obj 'empty?)
	...
	(cond ((infix-op? token) ;; arithmetic infix operators
		 ...
		 (if (ask val 'true?)
	      ...
	      ;B5 part one
	      ((in? token)
	       (let ((right (eval-item line-obj env)))
		 (ask right '__contains__ val)))
	      ...
	      ;B5 part two					
	      ((not? token)
	       (let ((inside? (ask line-obj 'next)))
		 (let ((right (eval-item line-obj env)))
		   (if (in? inside?)
		       (negate-bool (ask right '__contains__ val))))))
	      ;end of B5
...
(define (collect-key-value line-obj env close-token)
 (let ((token (ask line-obj 'peek)))
   (cond ((eq? token close-token) '() )   ;i need to create a huge cons of everything
	                                 ;in this, but how?
        
	 ((comma? token)
	  (let ((key (py-eval line-obj env)))
	    (let ((value (py-eval line-obj env)))
	      (begin (ask line-obj 'push token)                       
		     (cons (cons key value) (collect-key-value line-obj env close-token)))))) ; with the    pair at the front of the line-obj
	 (else (collect-key-value line-obj env close-token)) ))) ;how do i get it to call collect-key-value but with the next token in the line-obj?
...
;B7 make-if-block, if-block-pred, if-block-body, if-block-else, eval-if-block

;; Conditionals
(define (make-if-block line-obj env)
  (let ((pred (pred-helper line-obj))
	 (body (read-block (ask line-obj 'indentation) env)))
    (list '*BLOCK* '*IF-BLOCK* (cons (ask line-obj 'indentation) pred) (split-block body))))
...
(define (eval-if-block block env)
  (let ((if-block (py-eval (make-line-obj (if-block-pred block)) env)))
    (let ((bool (ask if-block 'true?)))
      (if bool
	  (eval-sequence (if-block-body block) env)
	  (let ((else-statement (if-block-else block)))
	    (if (equal? else-clause #f)
		*NONE*
		(eval-item (make-line-obj else-statement) env)) )))))
...
;B7 part 2 eval-else-block, make-elif-block, elif-block-pred, elif-block-body, elifo-block-else, eval-elif-block, elif-change
...
(define (make-elif-block line-obj env)
  (let ((pred (pred-helper line-obj))
	(body (split-block (read-block (ask line-obj 'indentation) env))))
    (list '*BLOCK* '*ELIF-BLOCK* (cons (ask line-obj 'indentation) pred) body)))

(define (elif-block-pred block) (caddr block))
(define (elif-block-body block) (car (cadddr block)))
(define (elif-block-else block) (car (cdr (cdr (cadddr block)))))

(define (eval-elif-block block env)
  (eval-if-block (elif-change block) env))

(define (elif-change block)
...
(define (pred-helper l-obj)
  (let ((up-next (ask l-obj 'next)))
    (if (eq? up-next ':)
	'()
	(cons up-next
	      (pred-helper l-obj)))))
...
(define (make-while-block line-obj env)
  ...
  (list '*BLOCK* '*WHILE-BLOCK* (cons (ask line-obj 'indentation) pred) (car body) (cdr body)))
  )
...
(define (make-for-block line-obj env)
   (let* ((pred (pred-helper line-obj))
	(body (split-block (read-block (ask line-obj 'indentation) env)))
	(var (car pred))
	(collect (cons '*DUMMY-INDENT* (cddr pred))))

         (list '*BLOCK* '*FOR-BLOCK* var collect (car body) (cdr body))))
...
(define (eval-for-block block env)
   (let* ((var (for-block-var block))
     ...
     (let* ((col-obj (py-eval (make-line-obj collection) env))
           (iter (ask col-obj '__iter__ var body env)))
	   (if (eq? iter '*BREAK*)
	          *NONE*
		 (if else-clause
		  (py-eval (make-line-obj else-clause) env)
		  *NONE*)))) )
