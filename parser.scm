...
(define (py-read)
  (define (get-indent-and-tokens)
    ;Question 2
     ...
     (define (get-indent)
      (define (get-indent-helper spaces)
       (let ((peek (peek-char)))
	 ;(print peek)
	 (if (eq? peek #\space)
	     (begin (read-char) (get-indent-helper (+ 1 spaces)))
	     spaces)))
      (get-indent-helper 0))
    (let ((indents (get-indent)))
    (cons indents (get-tokens '()))))
  ...
  (define (get-num num-so-far)
    ...
    (let ((char (peek-char)))
      (cond ((char-numeric? char)
	     (helper (word num-so-far (char->symbol (read-char))) bool))
 
	    ((and (equal? char #\.)
		  (equal? bool #f))
	     (helper (word num-so-far (char->symbol (read-char))) #t))
  ...
  (define (get-string type)
    ...
    ;; function is executed will be the first character of the desired string.
       ...
       (define (get-string-helper)
    (let ((char (read-char)))
      (if (eq? char type)
	   '()
	  (begin

	  (cons  char (get-string-helper) )))))
   (get-string-helper))
  ...
  (define (ignore-comment)
    ;Both Partners, Question 1
    (define (read-comment char)
      (cond ((char-newline? (peek-char)) 
	     (begin '*COMMENT-IGNORED*
		    (read-char)))
	    ((eof-object? (peek-char))
	     '*COMMENT-IGNORED*)
	    (else (read-comment (read-char)))))
    
    (read-comment (read-char)))
